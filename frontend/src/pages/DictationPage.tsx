...
export default function DictationPage() {
  ...
  // While dictation is listening, ping the app-level inactivity timer once a
  // minute. Hands-free dictation produces no mouse/keyboard events, and without
  // this the 30-minute HIPAA auto-logout would fire mid-dictation and lose the
  // transcript (same bug as recorded visits, fixed via the recording store).
  useEffect(() => {
    if (!isListening) return;
    const ping = () => window.dispatchEvent(new Event('pronote:activity'));
    ping();
  ...
  const handleGenerateNote = async () => {
    ...
    try {
      ...
      // Synthesize default styling/verbosity from the template's sections so the
      // AI honors bullet/paragraph + concise/detailed (see CapturePage for why).
      const effectiveSectionSettings =
        resolvedTpl?.sectionSettings ?? buildDefaultSectionSettings(resolvedTpl?.sections);
