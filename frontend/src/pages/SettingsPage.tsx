...
export default function SettingsPage() {
  ...
  // Whether the active subscription is already set to end at the period close.
  ...
  // Load whether the subscription is already scheduled to cancel, so we can show
  // either the "Cancel" or the "Reactivate" control.
  useEffect(() => {
    if (!user || user.subscriptionStatus === 'trial') return;
    subscriptionsApi
      .get()
      .then((info) => setCancelAtPeriodEnd(!!info?.subscription?.cancelAtPeriodEnd))
      .catch(() => {});
  ...
  const handleCancelSubscription = async () => {
    ...
    try {
      ...
      toast.success(res.message || 'Subscription will be canceled at the end of the billing period.', { duration: 6000 });
    } catch (error) {
      const err = error as { message?: string };
  ...
  const handleReactivateSubscription = async () => {
    ...
    try {
      ...
      toast.success(res.message || 'Subscription reactivated.', { duration: 5000 });
    } catch (error) {
      const err = error as { message?: string };
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          >
            ...
            <div className="bg-white/[0.04] border border-white/[0.08] rounded-2xl p-4 mt-4">
              ...
              >
                <CreditCard size={14} /> {user?.subscriptionStatus === 'trial' ? 'Upgrade Plan' : 'Change Plan'}
              </button>

              {/* Cancel / Reactivate — only for users on a paid plan */}
              {user?.subscriptionStatus !== 'trial' && (
                cancelAtPeriodEnd ? (
                  <div className="mt-3 pt-3 border-t border-white/[0.08]">
                    <p className="text-xs text-amber-400 mb-2 flex items-start gap-1.5">
                      <AlertCircle size={12} className="mt-0.5 flex-shrink-0" />
                      Your plan is set to cancel at the end of the current billing period. You keep access until then.
                    </p>
                    <button
                      className="w-full py-2 border border-emerald-500/30 bg-emerald-500/10 text-emerald-400 hover:bg-emerald-500/20 rounded-xl text-sm font-bold transition-all disabled:opacity-60"
                      onClick={handleReactivateSubscription}
                      disabled={isReactivating}
                    >
                      {isReactivating ? 'Reactivating…' : 'Keep My Subscription'}
                    </button>
                  </div>
                ) : (
                  <button
                    className="w-full mt-2 py-2 border border-red-500/30 bg-red-500/5 text-red-400 hover:bg-red-500/15 rounded-xl text-sm font-semibold transition-all flex items-center justify-center gap-2"
          ...
          <div className="space-y-4">
            <div className="p-4 bg-amber-50 border border-amber-200 rounded-lg">
              <p className="text-sm text-amber-800">
                ...
                you won't be charged again, and you'll keep full access until then. You can reactivate
