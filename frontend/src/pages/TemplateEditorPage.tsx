...
import { DEFAULT_CONTENT, DEFAULT_STYLING } from '../data/sectionDefaults';
...
// Per-section content hints and default styling live in ../data/sectionDefaults
// so the capture/dictation/upload flows synthesize the SAME defaults for built-in
// templates that carry no sectionSettings of their own.
...
export default function TemplateEditorPage() {
  ...
  // When editing a built-in template, keep the original name (no " - Copy"
  // suffix). The save handler will replace the built-in in the user's "My
  // Templates" list with the customized version, so showing the same name in
  // the picker is the correct mental model — there's still only ONE "SOAP
  // Note" in your list, just your edited version of it.
  const [templateName, setTemplateName] = useState(
    isCreate ? '' : (template ? template.name : 'New Template')
  );
  ...
  const handleSave = async () => {
    ...
    // ID of the built-in template the user is editing (if any). When present,
    // we REPLACE this built-in in My Templates with the customized version
    // below — the user only sees ONE "SOAP Note" in their picker (their
    // version), so there's no chance of recording with the original by
    // accident and losing all the bullet-point / verbosity customization.
    const replacedBuiltInId: string | null =
      !isCreate && template && !template.isCustom ? template.id : null;
    ...
    try {
      ...
      // Drop the built-in we just customized so only the custom version
      // appears in My Templates. The built-in still exists in the global
      // library and can be re-added from the Templates page if the user
      // wants both.
      const withoutBuiltIn = replacedBuiltInId
        ? prevAdded.filter(id => id !== replacedBuiltInId)
        : prevAdded;
      allIds = withoutBuiltIn.includes(saved.id) ? withoutBuiltIn : [...withoutBuiltIn, saved.id];
    ...
    toast.success(
      replacedBuiltInId
        ? `"${saved.name}" updated with your settings. Recording with this template will now use your bullet/verbosity preferences.`
        : `"${saved.name}" saved and added to My Templates!`,
      { duration: 5000 }
    );
    navigate('/templates');
  };
