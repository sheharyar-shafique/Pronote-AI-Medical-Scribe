-- ============================================================
-- PRONOTE COMPLETE DATABASE SCHEMA
-- Run this in your Supabase SQL Editor
-- This creates ALL tables for the complete website
-- ============================================================
...
CREATE TABLE IF NOT EXISTS subscriptions (
  ...
  payment_provider VARCHAR(50) DEFAULT 'stripe' CHECK (payment_provider IN ('stripe', 'paypal')),
...
-- ============================================
-- 4. NOTE CONTENTS TABLE (structured content)
-- ============================================
...
-- ============================================
-- 5. TEMPLATES TABLE (custom templates)
-- ============================================
...
-- ============================================
-- 9. NOTE FEEDBACK TABLE (for accuracy tracking)
-- ============================================
...
-- ============================================
-- 10. ACTIVITY LOG TABLE (for admin dashboard)
-- ============================================
...
-- ============================================
-- 11. PAYMENTS TABLE (payment history)
-- ============================================
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'appointments_owner_policy' AND tablename = 'appointments') THEN
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'note_feedback_owner_policy' AND tablename = 'note_feedback') THEN
    CREATE POLICY note_feedback_owner_policy ON note_feedback
      FOR ALL USING (user_id = auth.uid() OR EXISTS (SELECT 1 FROM users WHERE id = auth.uid() AND role = 'admin'));
  END IF;
END $$;
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'payments_owner_policy' AND tablename = 'payments') THEN
...
-- ============================================
-- STORAGE BUCKET FOR AUDIO FILES (run separately in Storage section)
-- ============================================
-- Go to Supabase Dashboard > Storage > Create new bucket named 'audio-files'
-- Set it to private
